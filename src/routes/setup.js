...
router.get('/db', async (req, res) => {
  try {
    ...
    // 1. DROP everything in the correct order (children first, then parents)
    await db.query(`DROP TABLE IF EXISTS audit_logs;`);
    await db.query(`DROP TABLE IF EXISTS dispatch_picks;`);
    await db.query(`DROP TABLE IF EXISTS dispatch_bins;`);
    await db.query(`DROP TABLE IF EXISTS dispatches;`);
    await db.query(`DROP TABLE IF EXISTS customer_strategies;`);
    await db.query(`DROP TABLE IF EXISTS validation_strategies;`);
    await db.query(`DROP TABLE IF EXISTS user_customers;`);
    await db.query(`DROP TABLE IF EXISTS customers;`);
    await db.query(`DROP TABLE IF EXISTS users;`);

    // 2. Recreate everything with TEXT for Nagare Time (Schedule Sent Date)
    const schema = `
    ...
    res.status(200).json({ message: "DATABASE RESET COMPLETE: All tables recreated with TEXT for Nagare Time!" });
  } catch (err) {
    console.error(err);
...
// Admin, Customer, and Link routes stay here...
