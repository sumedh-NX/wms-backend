...
router.get('/db', async (req, res) => {
  try {
    ...
    // 2. RECREATE EVERYTHING - ALL DATES ARE NOW TEXT
    const schema = `
      CREATE TABLE users (
      ...
      CREATE TABLE dispatches (
        ...
        ref_case_pack INT, 
        ref_supply_date TEXT,        // CHANGED TO TEXT
        ref_schedule_sent_date TEXT, // CHANGED TO TEXT
      ...
      CREATE TABLE dispatch_bins (
        ...
        schedule_sent_date TEXT,     // CHANGED TO TEXT
        schedule_number TEXT, 
        supply_quantity INT, 
        supply_date TEXT,            // CHANGED TO TEXT
        vendor_code TEXT, 
        invoice_number TEXT, 
        product_name TEXT, 
        unload_loc TEXT, 
    ...
    res.status(200).json({ message: "DATABASE TOTAL RESET: All date columns are now TEXT. No more range errors!" });
  } catch (err) {
    console.error(err);
...
// --- Setup Admin / Customer routes remain exactly the same ---
